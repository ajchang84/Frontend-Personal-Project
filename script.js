...
$(document).ready(function () {
      ...
      success: function (data) {
        ...
        if (data.name) {
          ...
          dataArray = [
            ...
            data.stats[2].base_stat, // sp_atk
            data.stats[1].base_stat, // sp_def
  ...
  function radarChart() {
    ...
    var axis = svgContainer
      ...
      .attr("x2", function (d, i) {
        return 100 * (1 - Math.sin((i * 2 * Math.PI) / 6));
      })
      .attr("y2", function (d, i) {
        return 100 * (1 - Math.cos((i * 2 * Math.PI) / 6));
      })
    ...
    for (var level = 0; level < 5; level++) {
      ...
      var levels = svgContainer
        ...
        .attr("x1", function (d, i) {
          return levelFactor * (1 - Math.sin((i * 2 * Math.PI) / 6));
        })
        .attr("y1", function (d, i) {
          return levelFactor * (1 - Math.cos((i * 2 * Math.PI) / 6));
        })
        .attr("x2", function (d, i) {
          return levelFactor * (1 - Math.sin(((i + 1) * 2 * Math.PI) / 6));
        })
        .attr("y2", function (d, i) {
          return levelFactor * (1 - Math.cos(((i + 1) * 2 * Math.PI) / 6));
        })
        .attr(
          "transform",
          "translate(" + (125 - levelFactor) + ", " + (125 - levelFactor) + ")"
        )
        .attr("stroke", "gray")
    ...
    var labels = svgContainer
      ...
      .attr("x", function (d, i) {
        return 100 * (1 - 1.2 * Math.sin((i * 2 * Math.PI) / 6));
      })
      .attr("y", function (d, i) {
        return 100 * (1 - 1.1 * Math.cos((i * 2 * Math.PI) / 6));
      })
      .attr("transform", "translate(25, 25)")
    ...
    // initialize
    var verticesA = svgContainer
      .selectAll(".verticesA")
      .data([0, 0, 0, 0, 0, 0])
      .enter()
      .append("svg:circle")
      .classed("verticesA", true)
      .attr("r", 3)
      .attr("cx", function (d, i) {
        return (
          125 *
          (1 -
            (parseFloat(Math.max(d, 0)) / 250) *
    ...
    var verticesB = svgContainer
      .selectAll(".verticesB")
      .data([0, 0, 0, 0, 0, 0])
      .enter()
      .append("svg:circle")
      .classed("verticesB", true)
      .attr("r", 3)
      .attr("cx", function (d, i) {
        return (
          125 *
    ...
    var polygonA = svgContainer
      .selectAll(".polygonsA")
      .data([0, 0, 0, 0, 0, 0])
      .enter()
      .append("svg:polygon")
      .classed("polygonsA", true)
      .classed("polygons", true)
      .attr("points", function (d) {
        var verticesString = "";
        [0, 0, 0, 0, 0, 0].forEach(function (d, i) {
          verticesString +=
            125 *
              (1 -
                (parseFloat(Math.max(d, 0)) / 250) *
    ...
    var polygonB = svgContainer
      .selectAll(".polygonsB")
      .data([0, 0, 0, 0, 0, 0])
      .enter()
      .append("svg:polygon")
      .classed("polygonsB", true)
      .classed("polygons", true)
      .attr("points", function (d) {
        var verticesString = "";
        [0, 0, 0, 0, 0, 0].forEach(function (d, i) {
  ...
  function drawPolygon(data, pokemon) {
    ...
    // draw coordinates
    var vertices = svgContainer
      .selectAll(".vertices" + pokemon)
      .data(data)
      .transition()
      .duration(1000)
      .attr("r", 3)
    ...
    // draw polygons
    var polygons = svgContainer
      .selectAll(".polygons" + pokemon)
      .data(data)
      .transition()
      .duration(1000)
      .attr("points", function (d) {
    ...
    d3.selectAll(".polygons")
      .on("mouseover", function (d) {
        svgContainer
          .selectAll(".polygons") // fade all other polygons out
          .transition(250)
          .attr("fill-opacity", 0.1)
  ...
  function score(data, pokemon) {
    $({ someValue: $("#score" + pokemon).text() }).animate(
      { someValue: data },
      {
        duration: 1000,
        easing: "swing",
